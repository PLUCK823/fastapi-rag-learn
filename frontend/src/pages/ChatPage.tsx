...
const EMPTY_CHAT = (
	<div className="text-center py-16 animate-fade-in-up">
		...
		<p
			className="display-text text-sm mb-1"
...
export default function ChatPage() {
	...
	const { messages, isStreaming, send, clear, prepareSend, resetLoadFlag } =
	...
	const filteredDocs = docFilter.trim()
		? docs.filter((d) =>
				d.filename.toLowerCase().includes(docFilter.trim().toLowerCase()),
			)
		: docs;
	...
	const displayName = activeSessionId
		? (sessions.find((s) => s.session_id === activeSessionId)?.first_question ??
			"新的对话")
		: "新的对话";
								...
								{editingDocId === d.id ? (
									<input
										...
										onBlur={() => {
											if (
												editingDocName.trim() &&
												editingDocName.trim() !== d.filename
											) {
												executeRenameDoc();
											} else {
												setEditingDocId(null);
